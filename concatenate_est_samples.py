...
def concat(type):
    f_files=np.load('Theta_est_fMRI/ThetaFiles.npy')
    d_files=np.load('Theta_est_dti/ThetaFiles.npy')
    flag=True
    if type=='fMRI':
        file=f_files
    else:
        file=d_files
    for i in file:      
        y=np.load('Samples_est_'+type+'/'+i[3:]+'.npy')
        if flag:
            wholeColl=y
            flag=False
        else:
            wholeColl=np.hstack((wholeColl,y))
    np.save('concat_est_'+type,wholeColl.T)
